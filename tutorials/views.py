...
def tutorial_list(request):
    if request.method == 'GET':
        ...
        result = v_array.tolist()
        # print(result)
        
        embed_json = json.dumps(result)
        # print(embed_json)
        ...
        Tutorial.objects.update_or_create(title=cow_id, defaults=data)

        # print(Tutorial.objects.all().filter('array'))
        # inf = Tutorial.objects.values_list('array', flat=True)
        inf = Tutorial.objects.values_list('title', 'array')
        print(np.array(inf))
        inf = np.array(inf)
        kho = inf.astype(float)
        # print(kho.shape)

        test = kho[1][:]
        # print(test)
        input1 = torch.from_numpy(kho[0][:])
        # print(input1)
        print('+++++++++++++++++++++++++++++++++++++++++++++')
        input2 = torch.from_numpy(kho[1][:])
        # print(input2)
        ...
        # ndarray = np.array(list(inf))

        # vlqs = inf.values_list('array', flat=True)
        # print(vlqs)
        # vlqs = int(vlqs)
        # votes = np.fromiter(vlqs)
        # print(votes)

        # input1 = torch.from_numpy(ndarray.iloc[:,1].values)
        # print(input1)

        
        

        # print(Tutorial.objects.all().values('title','array'))

        # return JsonResponse(table, safe=False)  
        return JsonResponse(embed_json, safe=False)
